...
type person struct {
	...
	Userid struct {
		Value    string `xml:",chardata"` // USERNAME
...
func Person(institution, syncID, username, password, givenname, familyname, birthday, email, street, locality, pcode, kindOfPersonStudentOrStaffOrParent string, tel []telefon, childOrParentIds []string, parent bool) person {
	a := new(person)
	a.Recstatus = "1"
	a.Sourcedid.Source = institution
	a.Sourcedid.ID = syncID
	a.Userid.Value = username
	a.Userid.Password = password
...
func (a *person) ParseToXML() ([]byte, string, error) {
	...
	if err != nil {
		err2 := errors.New(err.Error() + " Person: " + a.Userid.Value + " was not(!!!) imported. \n")
		return output, "", err2
	}
	log := "Person: " + a.Userid.Value + " is imported. \n"
